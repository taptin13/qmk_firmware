...
#define RF_REPORT_REPEAT_TIMEOUT_MS 2000
...
static void    uart_send_bytes(uint8_t *buffer, uint32_t length);
...
void uart_send_report_repeat(void) {
    ...
    if (timer_elapsed32(interval_timer) > 50) {
        ...
        if (no_act_time <= (RF_REPORT_REPEAT_TIMEOUT_MS / 10)) {
            uart_send_report(CMD_RPT_BYTE_KB, bytekb_report_buf, 8);
            wait_us(200);
...
void RF_Protocol_Receive(void) {
    ...
    if (Usart_Mgr.RXDState == RX_Done) {
        if (Usart_Mgr.RXDLen > 4) {
            /* A data frame is: header, command, type, length, payload, checksum. */
            if (RX_LEN > (UART_MAX_LEN - 5) || Usart_Mgr.RXDLen != (uint8_t)(RX_LEN + 5)) {
                Usart_Mgr.RXDState = RX_FORMAT_ERR;
                return;
            }
...
void uart_send_report(uint8_t report_type, uint8_t *report_buf, uint8_t report_size) {
    if (report_buf == NULL || report_size > (UART_MAX_LEN - 5)) {
        return;
    }
